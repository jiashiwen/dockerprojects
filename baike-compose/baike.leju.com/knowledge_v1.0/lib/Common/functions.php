...
function I($name,$default='',$filter=null,$datas=null) {
    ...
    if ( IS_CLI ) {
        ...
        foreach ( $argv as $i => $arg ) {
            $arg = explode('=', $arg, 2);
            $_GET[trim($arg[0])] = isset($arg[1]) ? trim($arg[1]) : '';
        }
    }
 ...
 * @return Think\Model
 */
function D($name='',$layer='',$module='Common') {
    ...
    if(class_exists($class)) {
        ...
        // var_dump($name, $layer, $module);
        // 自动加载公共模块下面的模型
        if(!C('APP_USE_NAMESPACE')){
            import('Common/'.$layer.'/'.$class);
        }else{
            if ( !in_array($module, array('Common', '')) && defined('BIND_MODULE') && constant('BIND_MODULE')!='' ) {
                $module = BIND_MODULE;
            }
            $class      =   '\\'.$module.'\\'.$layer.'\\'.$name.$layer;
        }
 ...
 * @return Think\Model
 */
function K($name='',$layer='') {
    if(empty($name) || empty($layer)) return false;
    static $_model  =   array();
    if(isset($_model[$name.$layer]))
        return $_model[$name.$layer];
    $class          =   parse_res_name($name,$layer);
