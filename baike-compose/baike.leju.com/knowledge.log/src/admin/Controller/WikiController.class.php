...
class WikiController extends BaseController {
	...
	public function index(){
		...
		// $newform['check'] = 2;
		//请求接口查询列表数据
		$url = C('DATA_TRANSFER_API_URL') . "api/item?mode=1&search=1&check=2";
		...
		echo '<!--', PHP_EOL, var_export($info, true), PHP_EOL, '-->', PHP_EOL;
		$this->dicts = C('DICT.WIKI');
	...
	protected function save ( $data=array() ) {
		if(strtotime($data['releasetime']) > NOW_TIME) {
 		...
 		$array = array(
 			'id' => '', 'entry' => '', 'pinyin' => '', 'brief' => '', 'from' => '', 'fromurl' => '', 
 			'content' => '', 'pic' => '', 'media' => '', 'creator' => '', 'creatorid' => '',

 			'category' => '', 'releasetime' => '',
 			'index_focus' => '', 'focus_title' => '', 'focus_pic' => '',
 			'index_company' => '', 'company_title' => '', 'company_pic' => '',
		...
		$action_type = in_array($new_form['action_type'], array('save', 'publish')) ? $new_form['action_type'] : 'save';
		$action_type = ( $action_type == 'publish' ) ? 0 : 1;
		unset($new_form['action_type']);
		if ( $action_type==1 ) {
			$new_form['state'] = 1;
		}
		if ( $action_type==0 ) {
			$new_form['state'] = $new_form['releasetime'] <= NOW_TIME ? 3 : 2;
		}
		// $new_form['from'] = ( $new_form['from'])
		
		$new_form['keywords'] = explode(',', $new_form['keywords']);
		// var_dump($action_type, $new_form); exit;
		$url = C('DATA_TRANSFER_API_URL') . "api/item?id={$data['id']}&check={$action_type}";
		$save_result = curl_post($url, json_encode($new_form));
		$api_result = json_decode($save_result['result'], true);

		$debug = array();
		$debug[] = $url;
		$debug[] = json_encode($new_form);
		$debug[] = $new_form;
		$debug[] = $api_result;
		// $save_result['result']['debug'] = $debug;
		var_dump($debug);
