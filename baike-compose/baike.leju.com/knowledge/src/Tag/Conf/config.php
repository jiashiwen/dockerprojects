...
return array(
	...
	'URL_ROUTE_RULES' => array( //定义路由规则
		// @ 标签聚合列表
		'agg/:tag/:page\d' => 'List/Tags?tag=:1&page=:2',	// PC 端 通过标签进行词条聚合列页
		'agg' => 'List/Tags',				// PC 端 通过标签进行词条聚合列页
		// 规则中的分隔符必须使用 / 处理，真实路由即 Query_String 中，可以使用 URL_PATHINFO_DEPR 参数中指定的分隔符来进行分隔
		'word/:id' => 'Show/index',				// 查看指定词条
		'album/:id' => 'Show/album',				// 查看指定词条
		'morenews/:id\d/:page\d' => 'Show/morenews?id=:1&page=:2',		// 查看指定词条
		'cate/:cateid\d' => 'List/index',		// 查看指定分类的词条
		// @ 词条列表页
		'listall/:page\d' => 'List/all',		// PC 端 可以通过 page 进行词条分页查看
		'listall' => 'List/all',				// 移动端 查看所有词条

		// 'search/:word-:page\d' => 'Search/index',	// 查看指定分类的词条 PC 端 按分页查看搜索结果
		'search/:word' => 'Search/index',	// 查看指定分类的词条 移动端 所有搜索结果
		'search' => 'Search/index',			// 查看指定分类的词条 通用 : search.html?word=\w&page=\d
