...
class PcPageLogic extends PageLogic {
			...
			foreach ($level2 as $id => $v)
			{
				...
				if ($result['list'])
				{
					...
					foreach($result['list'] as $k=>$item)
					{
						...
						// // 标签转换为 id 与标签名称
						// $_list[$k]['tagsinfo'] = $this->convertTagToTagid($_list[$k]['tags']);
						$tagids = trim($item['_origin']['tagids']);
						if ( $tagids != '' ) {
							$_lisg[$k]['tagsinfo'] = $lTags->getTagnamesByTagids($tagids);
						} else {
							// 标签转换为 id 与标签名称
							// $_lisg[$k]['tagsinfo'] = $this->convertTagToTagid($_lisg[$k]['tags']);
							$_lisg[$k]['tagsinfo'] = array();
						}
					}
					...
					foreach ($level3 as $sid => $si)
					{
						...
						if ($result['list'])
						{
							...
							foreach( $result['list'] as $k=>$item )
							{
								...
								$tagids = trim($item['_origin']['tagids']);
								if ( $tagids != '' ) {
									$ls[$k]['tagsinfo'] = $lTags->getTagnamesByTagids($tagids);
								} else {
									// 标签转换为 id 与标签名称
									// $ls[$k]['tagsinfo'] = $this->convertTagToTagid($ls[$k]['tags']);
									$ls[$k]['tagsinfo'] = array();
								}
							}
	...
	protected function getCateKonwledgeList($id=0,$page=1,$pagesize=10,$city_en='bj',$city_cn='北京')
	{
		...
		if ($result['pager']['total'] > 0)
		{
			foreach ($result['list'] as $k => $ii)
			{
				...
				$list[$k]['cateid'] = $ii['_origin']['cateid'];
				$list[$k]['cover'] = $ii['_origin']['cover'];
				$list[$k]['content'] = mystrcut(clear_all($ii['_origin']['content']),80);
				$list[$k]['tags'] = explode(' ',$ii['_origin']['tags']);
				$tagids = trim($ii['_origin']['tagids']);
				if ( $tagids != '' ) {
					$list[$k]['tagsinfo'] = $lTags->getTagnamesByTagids($tagids);
				} else {
					// 标签转换为 id 与标签名称
					// $list[$k]['tagsinfo'] = $this->convertTagToTagid($list[$k]['tags']);
					$list[$k]['tagsinfo'] = array();
				}
	...
	private function linkopts($form, $total, $listtype='cate')
	{
		...
		if ( $listtype=='cate' ) {
			$linkstring = !empty($linkopts) ? 
							url('cate', array('id'=>$form['id'], 'page'=>'#'), 'pc', 'baike')
						  :
							url('cate', array(), 'pc', 'baike');
		}
		if ( $listtype=='agg' ) {
			$linkstring = !empty($linkopts) ? 
							url('agg', array('tag'=>$form['tagid'], 'id'=>$form['id'], 'page'=>'#'), 'pc', 'baike')
						  :
	...
	public function getAggPage($tag,$page,$city_cn,$cateid,$form)
	{
		...
		if ($result['pager']['total'] > 0)
		{
			foreach ($result['list'] as $key => $item)
			{
				...
				$list[$key]['cateid'] = $item['_origin']['cateid'];
				...
				$tagids = trim($item['_origin']['tagids']);
				if ( $tagids != '' ) {
					$list[$key]['tagsinfo'] = $lTags->getTagnamesByTagids($tagids);
				} else {
					// 标签转换为 id 与标签名称
					// $list[$key]['tagsinfo'] = $this->convertTagToTagid($list[$key]['tags']);
					$list[$key]['tagsinfo'] = array();
				}
	...
	public function getSearchPage($page,$keyword,$city_cn,$city_en,$id)
	{
		...
		if ($result['pager']['total']) {
			...
			$form = array('id'=>$id, 'page'=>$page, 'pagesize'=>$this->search_pagesize, 'keyword'=>$keyword);
