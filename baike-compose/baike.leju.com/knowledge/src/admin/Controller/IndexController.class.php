...
class IndexController extends BaseController {
	...
	protected function getWikiStats() {
		//统计图
		$words = $this->getWikiHotRank();
		// $this->assign('touch', $this->touch_domain());
		if ( defined('APP_DEPLOY') && APP_DEPLOY=='dev' ) {
			echo '<!--', PHP_EOL, print_r($words, true), PHP_EOL, '-->', PHP_EOL;
		}
	...
	/**
	 * 获取百科 七天热词排行
	 * @author cnwangyl <yongliang1@leju.com>
	 */
	protected function getWikiHotRank() {
		$num = 8;
		$cycle = 7;
		$endpoint = strtotime('today 00:00:00');
		$wiki_rank = D('VisitStats', 'Model', 'Common')->statsWikiHotRank($num, $cycle, $endpoint);
		return $wiki_rank;
	}
	...
	public function uptrend()
	{
		...
		$result = array('status'=>true, 'info'=>$count);
		$this->ajax_return($result);
	}
	...
	public function rank()
	{
		...
		$result = array('status'=>true, 'info'=>$list);
		$this->ajax_return($result);
	}
	...
	public function statistics()
	{
		...
		if($statistics) {
			$result = array('status'=>true,'info'=>$statistics);
			$this->ajax_return($result);
		} else {
			$this->ajax_error('暂无数据');
		}
	}
	...
	protected function getStatistics() {
		...
		$result = [
			'hits' => $mVisitStats->statsWikiHotTrend($cycle, $endpoint),
			'updates' => $mVisitStats->statsWikiUpdate($cycle, $endpoint),
			'words' => [],
		];
