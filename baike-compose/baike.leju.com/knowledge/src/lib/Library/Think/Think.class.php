...
class Think {
    ...
    static public function start() {
      ...
      if(C('CHECK_APP_DIR')) {
          ...
          // var_dump('CHECK_APP_DIR', C('CHECK_APP_DIR'), $module);
    ...
    static public function halt($error) {
        $e = array();
        // var_export(array('APP_DEBUG'=>APP_DEBUG, 'IS_CLI'=>IS_CLI));
        if (APP_DEBUG || IS_CLI) {
            //调试模式下输出错误信息
            if (!is_array($error)) {
        ...
        $exceptionPage =  C('EXCEPTION_PAGE');
        // var_export(get_included_files());
        if ( $exceptionPage && substr($exceptionPage, 0, 4) == 'http' ) {
            send_http_status(303);
            // 如果设置为目标网址，则直接跳转
            header("Location: {$exceptionPage}", TRUE, 303); // see other
            exit;
        } else {
            send_http_status(404);
            ...
            // }
            if ( DEFAULT_THEME!==C('DEFAULT_THEME') ) {
              $exceptionFile = str_replace(':theme:', C('DEFAULT_THEME'), $exceptionFile);
            }
            include $exceptionFile;
        }
        // var_export($exceptionFile);
        // var_export(C());
        exit;
    }
