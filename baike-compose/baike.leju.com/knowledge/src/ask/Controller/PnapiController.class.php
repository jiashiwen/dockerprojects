...
class PnapiController extends BaseController {
	...
	public function uPerson() {
		...
		if ( $direct !== 0 ) {
			// 非 0 进行关注操作
			$ret = $this->lUserCenter->AttentionWiki($userid, $person_id, $person['title']);
			if ( $this->lUserCenter->getResult() ) {
				$mPersons->where($where)->setInc('favs', 1);
				$msg = '公司关注成功';
			} else {
			...
			// 0 时进行取消关注操作
			$ret = $this->lUserCenter->UnAttentionWiki($userid, $person_id, $person['title']);
			if ( $this->lUserCenter->getResult() ) {
				$where['i_attention'] = ['gt', 0];
				$mPersons->where($where)->setDec('favs', 1);
				$msg = '取消公司关注成功';
			} else {
				$error = $this->lUserCenter->getError();
				$msg = $error['msg'];
		...
		// $ret = $mPersons->where($where)->field('favs')->find();
	...
	public function uQuestion() {
		...
		$question = $mPersonQuestions->find($question_id);
		if ( !$question ) {
		...
		$person = $mPersons->CheckPersonStatus($person_id);
		if ( !$person ) {
			$this->ajax_error('指定的人物暂时未提供乐道服务');
		}
		...
		if ( $direct !== 0 ) {
			...
			if ( $this->lUserCenter->getResult() ) {
				$mPersonQuestions->where($where)->setInc('i_attention', 1);
				$msg = '问题关注成功';
			} else {
				$error = $this->lUserCenter->getError();
				$msg = $error['msg'];
			...
			if ( $this->lUserCenter->getResult() ) {
				$where['i_attention'] = ['gt', 0];
				$mPersonQuestions->where($where)->setDec('i_attention', 1);
				$msg = '取消问题关注成功';
			} else {
				$error = $this->lUserCenter->getError();
				$msg = $error['msg'];
		...
		if ( $this->lUserCenter->getResult() ) {
			...
			$ret = $mPersonQuestions->where($where)->field('i_attention')->find();
	...
	public function uAnswer() {
		...
		$answer = $mPersonAnswers->find($answer_id);
		if ( !$answer ) {
		...
		if ( $direct !== 0 ) {
			...
			if ( $this->lUserCenter->getResult() ) {
				$mPersonAnswers->where($where)->setInc('i_attention', 1);
				$msg = '回答关注成功';
			} else {
				$error = $this->lUserCenter->getError();
				$msg = $error['msg'];
			...
			if ( $this->lUserCenter->getResult() ) {
				$where['i_attention'] = ['gt', 0];
				$mPersonAnswers->where($where)->setDec('i_attention', 1);
				$msg = '取消回答关注成功';
			} else {
				$error = $this->lUserCenter->getError();
				$msg = $error['msg'];
		...
		if ( $this->lUserCenter->getResult() ) {
			...
			$ret = $mPersonAnswers->where($where)->field('i_attention')->find();
			$fav = intval($ret['i_attention']);
		}
		$this->ajax_return(['status'=>true, 'msg'=>$msg, 'favs'=>$fav]);
	}
	...
	public function uGoodAnswer() {
		...
		if ( $direct !== 0 ) {
			...
			if ( $this->lUserCenter->getResult() ) {
				$mPersonAnswers->where($where)->setInc('i_good', 1);
				$msg = '问题点赞成功';
			} else {
				$error = $this->lUserCenter->getError();
				$msg = $error['msg'];
			...
			if ( $this->lUserCenter->getResult() ) {
				$where['i_good'] = ['gt', 0];
				$mPersonAnswers->where($where)->setDec('i_good', 1);
				$msg = '取消问题点赞成功';
			} else {
				$error = $this->lUserCenter->getError();
				$msg = $error['msg'];
		...
		if ( $this->lUserCenter->getResult() ) {
			...
			$ret = $mPersonAnswers->where($where)->field('i_good')->find();
	...
	public function ask() {
		...
		G('checking_start');
		$form = I('post.');
		$fields = array_flip(['title', 'desc', 'images', 'person_id']);
		$form = array_intersect_key($form, $fields);

		$person_id = intval($form['person_id']);
		if ( $person_id <= 0 ) {
			$this->ajax_error('请指定当前提问人物');
		}
		$mPersons = D('Persons', 'Model', 'Common');
		$person = $mPersons->getPersonInfo($person_id);
		if ( !$person ) {
			$this->ajax_error('请指定当前提问人物.');
		}
		...
		// 添加至数据库
		$mPersonQuestions = D('PersonQuestions', 'Model', 'Common');
		$id = $mPersonQuestions->data($form)->add();
		if ( $id ) {
			$form['id'] = $id;
			$mPersons->updateRelQuestions($person_id, $form['status']);
	...
	public function reply() {
		...
		// $mPersonAnswers = D('PersonAnswers', 'Model', 'Common');
		// $where = array('question_id'=>$qid, 'userid'=>$userid);
		// if ( $mPersonAnswers->where($where)->count()>0 ) {
		...
		$form['person_id'] = $question['person_id'];
		...
		// 插入回答数据
		$mPersonAnswers = D('PersonAnswers', 'Model', 'Common');
		$id = $mPersonAnswers->data($form)->add();
		if ( $id ) {
			G('business_start');
			...
			if ( $form['status']==2 ) {
				...
				$ret = $mPersonQuestions->updateQuestionNewAnswer($question['id']);
