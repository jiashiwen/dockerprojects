...
class CategoriesModel extends Model {
	...
	public function getCateList($ids, $type='kb') {
		if (!is_array($ids)) {
			$ids = (array)$ids;
		}
		
		$condition = array('parent'=>array('in',$ids),'type'=>$type,'status'=>0);
	...
	public function getMaxIorder($id, $type='kb')
	{
		$condition = array('type'=>$type,'parent'=>$id);	
	...
	public function getCateInfo($id, $type='kb') {
		...
		$condition = array('type'=>$type,'id'=>$id);	
	...
	public function getAllCate( $type='kb' ) {
		$allowed = array('kb', 'qa');
		...
		return $this->where($where)->order('level asc,iorder asc')->select();
	}

	//for Front
	public function getValidAllCate( $type='kb' ) {
		$where = array(
			'type' => $type,
			'status' => 0,
		);
		$order = 'level asc,iorder asc';
