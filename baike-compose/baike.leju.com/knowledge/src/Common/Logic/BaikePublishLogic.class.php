...
class BaikePublishLogic {
	...
	public function Publish ( $data=array() ) {
		...
		if ( $this->act=='insert' ) {
			...
			$data['id'] = $this->id = intval($model->getLastInsID());
		}
		// 更新 发布
		if ( $this->act=='update' ) {
	...
	public function batchPublish ( $datalist=array() ) {
		...
		foreach ( $datalist as $i => $item ) {
			$list[$i] = $this->pushKonwledge($item);
		}
		...
		$lSearch = D('Search', 'Logic', 'Common');
		$ret = $lSearch->createKnowledge($list);
		// if ( $ret ) {
		// 	// 保存历史数据
		// 	$history = $this->getModel('history');
		// 	$ret = $history->data($data)->add();
	...
	{
		$filter = array (
			'id'=>'','title'=>'','content'=>'','scope'=>'','cover'=>'','coverinfo'=>'',
			'editorid'=>'','cateid'=>'','catepath'=>'','src_url'=>'','ptime'=>'','top_title'=>'',
			'top_time'=>0,'top_cover'=>'','top_coverinfo'=>'','rcmd_time'=>0,'rcmd_title'=>'',
			'rcmd_cover'=>'','rcmd_coverinfo'=>'','status'=>'','utime'=>'','ctime'=>'',
			'version'=>'','src_type'=>'','top_coverinfo'=>'','rel_news'=>'','rel_house'=>'',
			'tags'=>'','editor'=>'','tagids'=>',,',
		);
	...
	public function getBaikeId() { return $this->id; }
