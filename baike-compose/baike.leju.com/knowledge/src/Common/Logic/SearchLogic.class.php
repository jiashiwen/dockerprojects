...
class SearchLogic {
	...
	public function getToken() {
		...
		if ( $ret['status'] ) {
			...
			debug('get token 接口异常', $ret, false, true);
		}
		$this->_token = $result;
		return $result;
	}
	...
	/* -------------- 字典和分析类操作 -------------- */
		...
		public function analyze ( $content, $stats=true, $limit=0, $dict='' ) {
			...
			if ( $ret['status'] ) {
				...
				debug('analyze 接口日志', $ret, false, true);
				if ( $ret['status']==true ) {
					$result = $ret['result'];
				}
			} else {
				debug('analyze 接口异常', $ret, false, true);
			}
		...
		public function getDictWords($dict='') {
			...
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('get dict 接口日志', $ret, false, true);
				if ( $ret['status']==true ) {
					$result = $ret['exists']['words'];
				}
			} else {
				debug('get dict 接口异常', $ret, false, true);
			}
			return $result;
		}
		...
		public function setDictWords($words=array(), $dict='') {
			...
			// var_export(array('api'=>$api, 'params'=>$params, 'headers'=>$headers, 'ret'=>$ret));
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('set dict 接口日志', $ret, false, true);
				if ( $ret['status']==true ) {
					$result = intval($ret['count']);
				}
			} else {
				debug('set dict 接口异常', $ret, false, true);
			}
			return $result;
		}
		...
		public function appendDictWords($words=array(), $dict='') {
			...
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('append dict 接口日志', array('params'=>$params,'ret'=>$ret), false, true);
				if ( $ret['status']==true ) {
					$result = intval($ret['count']);
				}
			} else {
				debug('append dict 接口异常', $ret, false, true);
			}
			return $result;
		}
		...
		public function removeDictWords($words=array(), $dict='') {
			...
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('remove dict 接口日志', $ret, false, true);
				if ( $ret['status']==true ) {
					$result = intval($ret['count']);
				}
			} else {
				debug('remove dict 接口异常', $ret, false, true);
			}
			return $result;
		}
		...
		public function existsDictWord($word='', $dict='') {
			...
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('exists dict 接口日志', $ret, false, true);
				if ( $ret['status']==true ) {
					$exists = count($ret['exists']['words']);
					$result = $exists > 0 ? true : false;
				}
			} else {
				debug('exists dict 接口异常', $ret, false, true);
			}
			return $result;
		}
		...
		public function countDictWord($dict='') {
			...
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('count dict 接口日志', $ret, false, true);
				if ( $ret['status']==true ) {
					$result = intval($ret['count']);
				}
			} else {
				debug('count dict 接口异常', $ret, false, true);
			}
			return $result;
		}
	...
	/* -------------- 搜索查询类操作 -------------- */
		...
		public function select(
			...
			debug('搜索服务接口调用', $dbg, false, true);
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('mutil 接口日志', $ret, false, true);
				$result = array(
					'pager' => $ret['pager'],
					'list' => $ret['result'],
				);
			} else {
				debug('mutil 接口异常', $ret, false, true);
			}
		...
		public function search(
			...
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('search 接口日志', $ret, false, true);
				$result = array(
					'pager' => $ret['pager'],
					'list' => $ret['result'],
				);
			} else {
				debug('search 接口异常', $ret, false, true);
			}
			return $result;
		}
	...
	/* -------------- 文档管理类操作 -------------- */
		...
		protected function _doc( $doc=array(), $index='', $type='', $method='create' ) {
			if ( !$doc ) {
				debug('文档接口 主键或文档内容缺失', $doc, false, true);
				return false;
			}
			$this->checkToken();
			...
			if ( !array_key_exists($method, $methods) ) {
				debug('文档接口 操作方法 错误', $method, false, true);
				return false;
			}
			// 验证业务索引是否允许
			$allowed = array(
			...
			if ( !array_key_exists($index, $allowed) ) {
				debug('文档接口 Indeces 索引 错误', $index, false, true);
				return false;
			}
			// 验证业务类型是否允许
			$type = trim($type);
			if ( $type=='' ) {
				...
				debug('文档接口 Type 类型 错误', $type, false, true);
				return false;
			} // in_array allowd
			...
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('doc 文档接口日志', $ret, false, true);
				$result = $ret['status'];
			} else {
				debug('doc 文档接口异常', $ret, false, true);
			}
			return $result;
		}
		...
		public function knowledgeDocConvert( $record=array(), $create=false, $batch=false ) {
			if ( empty($record) || !is_array($record) ) {
				debug('请传入要转换文档的数据记录', $record, false, true);
				return false;
			}
			$must_fields = array(
			...
			if ( isset($record['title']) && (
					!isset($record['title_firstletter'])
					||
					!isset($record['title_pinyin'])
					||
					!isset($record['url'])
				) ) { 
					$py = D('Pinyin', 'Logic', 'Common')->get_pinyin($record['title']);
					$str = ucfirst($py);
					$record['title_firstletter'] = substr($str, 0,1);
					$record['title_pinyin'] = $py;
					$record['url'] = url('show', array('id'=>$record['id']), 'pc', 'baike');
			}
			...
			if ( $create ) {
				...
				if ( count($tmp)!=count($must_fields) ) {
					debug(
						'请填写必填字段,包括('.implode(',', array_keys($must_fields)).') 而数据是 '.var_export($record, true),
						$must_fields, false, true
					);
					return false;
				}
			}
			...
			if ( isset($record['scope']) ) {
				...
				if ( !array_key_exists($record['scope'], $cities) ) {
					debug('没有指定的城市', $record['scope'], false, true);
					return false;
				}
		...
		public function wikiDocConvert( $record=array(), $create=false, $batch=false ) {
			...
			$must_fields = array(
				'title'=>'','content'=>'','tagids'=>'','status'=>'',
				'ctime'=>'','cateid'=>'','src_type'=>'',
				'id'=>'','editor'=>'','editorid'=>'',
				'pinyin'=>'',
			);
			if ( !isset($record['firstletter']) && trim($record['title'])!='' ) {
				$str = D('Pinyin', 'Logic', 'Common')->get_pinyin($record['title']);
				$record['firstletter'] = strtoupper(substr($str, 0, 1));
			}
			// 扩展
			$remove_fields = array(
				// 'url'=>'',
			...
			if ( $create ) {
				...
				if ( count($tmp)!=count($must_fields) ) {
					debug(
						'WIKI请填写必填字段,包括('.implode(',', array_keys($must_fields)).')',
						$must_fields, false, true
					);
					// var_dump($record, $tmp, $must_fields);
					return false;
				}
			}
			...
			$result['_scope'] = '全部';
			$create && $result['_hits'] = intval($record['hits']);
			$create && $result['_category'] = '百科词条';
			$_id && $result['_id'] = $_id;
			isset($record['title']) && $result['_title'] = $record['title'];
			$content = [];
			isset($record['summary']) && array_push($content, clear_all($record['summary']));
			isset($record['content']) && array_push($content, clear_all($record['content']));
			$result['_content'] = implode(PHP_EOL, $content);
			isset($record['tags']) && $result['_tags'] = trim(str_replace(',', ' ', $record['tags']));
			...
			isset($record['src_type']) && $_multi['src_type'] = $record['src_type'];
			isset($record['editor']) && $_multi['editor'] = $record['editor'];
			isset($record['title']) && $_multi['title_prefix'] = trim($record['title']);
			...
			$record['album'] = json_decode($record['album'], true);
			$record['extra'] = json_decode($record['extra'], true);
			$record['rel'] = json_decode($record['rel'], true);
			$record['seo'] = json_decode($record['seo'], true);
			$record['basic'] = json_decode($record['basic'], true);
			$result['_origin'] = $record;
		 ...
		 * @return array 保存文档时的数据结构
		 */
		public function questionDocConvert( $record=array(), $create=false, $batch=false ) {
			if ( empty($record) || !is_array($record) ) {
				debug('请传入要转换文档的数据记录');
				return false;
			}
			...
			if ( $create ) {
				...
				if ( count($tmp)!=count($must_fields) ) {
					debug(
						'请填写必填字段,包括('.implode(',', array_keys($must_fields)).')',
						$must_fields, false, true
					);
					return false;
				}
			...
			isset($record['content']) && $result['_content'] = $record['content'];
			isset($record['tags']) && $result['_tags'] = trim(str_replace(',', ' ', $record['tags']));
			isset($record['status']) && $result['_deleted'] = in_array($record['status'], array(21,22,23)) ? false : true;
			isset($record['ctime']) && $result['_doccreatetime'] = intval($record['ctime'])*1000;
			$ptime = isset($record['ptime']) ? intval($record['ptime']) : NOW_TIME;
			$result['_docupdatetime'] = $ptime*1000;
			isset($record['cateid']) && $_multi['cateid'] = $record['cateid'];
			...
			isset($record['editor']) && $_multi['editor'] = $record['editor'];
			isset($record['title']) && $_multi['title_prefix'] = $record['title'];
			$_multi['src_type'] = $record['source'];
			$lPinyin = D('Pinyin', 'Logic', 'Common');
			$_multi['title_pinyin'] = $lPinyin->get_pinyin($record['title']);;
			$_multi['title_firstletter'] = ucfirst(substr($_multi['title_pinyin'], 0,1));
			isset($record['url']) && $result['_url'] = $record['url'];
			!empty($_multi) && $result['_multi'] = $_multi;
			foreach ( $remove_fields as $field => $item ) {
				if ( isset($record[$field]) ) {
			...
			if ( is_string($record['data']) && substr($record['data'], 0, 1)=='{' ) {
			...
			$result['_origin'] = $record;
			$result['_category'] = '问答';
			$result['_hits'] = intval($record['i_hits']);
		...
		public function batchesUpdate( $opts=array(), $prefix=array(), $doc=array(), $index='knowledge' ) {
			...
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('updatebyquery 接口日志', $ret, false, true);
				$result = $ret['status'];
			} else {
				debug('updatebyquery 接口异常', $ret, false, true);
			}
			return $result;
		}
		...
		public function batchesRemove( $opts=array(), $prefix=array(), $index='knowledge' ) {
			...
			if ( $ret['status'] ) {
				$ret = json_decode($ret['result'], true);
				debug('removebyquery 接口日志', $ret, false, true);
				$result = $ret['status'];
			} else {
				debug('removebyquery 接口异常', $ret, false, true);
			}
			return $result;
		}
		/**
		 ...
		 *
		 */
		public function getRecommendBaike ( $total=5, $prefix=array(), $opts=array(), $keyword=false, $fields=array(), $model='random' ) {
			$total = intval($total);
			...
			$opts = $this->parseWhere($opts);
			$prefix = $this->parseWhere($prefix);
			$fields = empty($fields) ? array('_id','_tags','_title','_multi.catepath','_origin.scope', '_origin.cover') : $fields;
	...
	/* -------------- 寰宇 提供的接口操作 -------------- */
		 ...
		 * 为词条&百科正文内容中的词库词加超链接
		 */
		public function renderingContent($content, $n=0, $exp=array(), $type='touch') {
			$types = array('touch', 'pc');
			if ( !in_array($type, $types) ) {
				$type = 'touch';
			}

			if ( is_string($exp) ) { $exp = (array)$exp; }

			$words = array();
			$cutwords = $this->analyze(strip_tags($content), true, 10, 'dict_wiki');
			foreach ( $cutwords as $k => $v ) {
				$word = trim($v['word']);
				if ( array_key_exists($word, $exp) ) {
					continue;
				}
			...
			if($cutwords) {
				...
				$where =array(
					'title'=>array('in', $words),
					'status'=>9,
				);
				$list = array();
				$ret = $mWiki->field($fields)->where($where)->select();
				...
				for ($i = 0, $k = count($cutwords); $i < $k; $i++) {
					if ( !array_key_exists($cutwords[$i]['word'], $list) ) {
						unset($cutwords[$i]);
					} else {
						$cutwords[$i]['id'] = intval($list[$cutwords[$i]['word']]['id']);
					}
					for ($j = $i + 1; $j < $k; $j++) {
				...
				foreach($cutwords as $v)
				{
					...
					$temp_word2[$md5] = "<a href='".url('show', array('word'=>$v['id'], $v['cateid']), $type, 'wiki')."'>{$v['name']}</a>";
				}
	 ...
	 */
	public function pushToBaidu( $urls=array(), $business='baike' ) {
		if ( empty($urls) ) {
			return false;
		}
		$apis = C('BAIDU_PUSH');
