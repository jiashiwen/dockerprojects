...
class KnowledgeController extends BaseController {
	...
	protected function formatCondition($get,$cities)
	{
		...
		if ($get['status'] && in_array($get['status'],array(0,1,9)))
		{
			$condition['status'] = $get['status'];
		}
		else
		{
			$condition['status'] = array('in',array(0,1,9));
		}
	...
	{
		$filter = array (
        	'id'=>'','title'=>'','content'=>'','scope'=>'','cover'=>'','coverinfo'=>'','editorid'=>'','cateid'=>'','catepath'=>'','src_url'=>'','ptime'=>'','top_title'=>'','top_cover'=>'','top_coverinfo'=>'','rcmd_title'=>'','rcmd_cover'=>'','rcmd_coverinfo'=>'','version'=>'','ctime'=>'','top_time'=>'','rcmd_time'=>'','editor'=>'','src_type'=>'',
			//extra
    	...
    	$data['src_type'] = $data['src_type'] == 1 ? 1 : 0;
	...
	public function edit() {
		...
		if ( IS_POST ) {
			...
			$this->verifyCityOp($data['scope'], $role['roleCitys']);
			$mKnowledge = D('Knowledge', 'Model', 'Common');
			$exist = $mKnowledge->where(array('cateid'=>$data['cateid'],'scope'=>$data['scope'],'id'=>array('neq',$data['id']),'title'=>$data['title']))->find();
			if ($exist)
			{
	...
	public function testPushNewsPool()
    {
        $id = I('get.id',0,'intval');
        $lInfos = D('Infos','Logic','Common');
        $opts = array("{id@eq}$id");
        //$opts = array('{id@eq}420598');
        //$a = $lInfos->pushNewsPool($data=array('id'=>388800),true);
        $result = $lInfos->selectNews('knowledge', 1, 1, $opts);
        var_dump($result);exit;
        $a = $lInfos->pushNewsPool($data);
        $data = array (
            'id' => 420598,
            'cateid' => 6,
            'catepath' => '0-1-5-6',
            ...
            'status' => 9,
            'title' => '知识标签二民族大道',
            'content' => '<p></p><p><br/></p><p><br/></p><p>知识标签一民族大道知识标签一民族大道</p><p><br/></p><p><br/></p><p></p>',
            'cover' => 'http://src.leju.com/imp/imp/deal/dc/1c/8/416915eb0ce7cde990a093014e1_p58_mk61_sX0_rt0_c235X176X47X22.jpg',
            'editor' => '袁海英',
            'ctime' => 1481019025,
            'ptime' => 1481089879,
            'utime' => 1481089879,
            'scope' => 'bj',
            'tags' => '民族大道 房产',
            'rel_news' => '[{"title":"\u591a\u5730\u697c\u5e02\u91cf\u4ef7\u9f50\u8dcc\u523a\u5934\u57ce\u5e02\u6216\u4e8c\u5ea6\u51fa\u624b\u4e25\u8c03","url":"http:\/\/bj.house.sina.com.cn\/news\/2016-11-11\/07196202620609223242635.shtml"}]',
            'rel_house' => '[{"site":"bj","hid":"3906","name":"\u7ea2\u73ba\u53f0","url":"http:\/\/house.leju.com\/bj3906"}]',
        );
        var_dump($a);
    }


    public function handCateDel()
    {
        if ( IS_POST )
        {
           $id = I('post.id',0,'intval');
            if ($id)
            {
                ...
                {
                    $lCate = D('Cate','Logic','Common');
                    if (in_array($cate['status'],array(0,1)))
                    {
                        $where['id'] = $id;
