...
class ShowController extends BaseController {
	...
	public function index() {
		...
		$r = S(C('REDIS'));
		$detail = $r->get($this->_cache_keys['detail'].$id);
		if($detail)
		{
		...
		else
		{
			...
			$r->set($this->_cache_keys['detail'].$id, $detail, 86400);
		}
		...
		$level2_page = ($this->_device == 'pc') ? 'pc_ct_search' : 'wd_info';
		...
		$seo = $seoLogic->wiki_detail($detail['entry'], $detail['content']);
