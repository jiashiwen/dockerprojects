...
class SearchLogic {
	...
	/* -------------- 文档管理类操作 -------------- */
		 ...
		 */
		public function batchesUpdate( $opts=array(), $doc=array(), $index='knowledge' ) {
		 ...
		 * @return bool 成功返回 true 失败返回 false
		 */
		public function batchesRemove( $opts=array(), $index='knowledge' ) {
			if ( empty($opts) ) {
				return false;
			}
			$this->checkToken();
	...
	/* -------------- 寰宇 提供的接口操作 -------------- */
		 ...
		 * 为词条&百科正文内容中的词库词加超链接
		 */
		public function renderingContent($content, $n = 0, $plant = null) {
			if($plant === 'b')
			{
			...
			if($cutwords)
			{
				...
				foreach($cutwords as $v)
				{
					...
					{
						$temp_word2[$md5] = "<a href='".url('show', array('word'=>base64_encode($v['id'])), 'touch', 'wiki')."'>{$v['name']}</a>";
					}
					else 
					{
						$temp_word2[$md5] = "<a href='".url('show', array('word'=>base64_encode($v['id'])), 'pc', 'wiki')."'>{$v['name']}</a>";
					}
				}
