...
<div class="content_wrapper b_wrapper <?php echo ($jsflag); ?>">
	<h2 class="b_title d-mt20"><?php echo ($binds["cate"]); ?></h2>
	<div class="d-tabbox">
		<ul class="d-nav">
		<?php if(is_array($binds["catelist"]["child"])): foreach($binds["catelist"]["child"] as $k=>$item): ?><li><a class="<?php if(($k) == $binds["cid"]): ?>d-cur<?php endif; ?>" href="<?php echo url('cate', array('id'=>$k), 'touch', 'baike'); ?>"><?php echo ($item["name"]); ?></a></li><?php endforeach; endif; ?>
		</ul>
		<div class="d-pane">
			<div class="d-panebox">
			<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><h3><?php echo ($item["name"]); ?></h3>
				<?php if(is_array($item["list"])): $i = 0; $__LIST__ = $item["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ii): $mod = ($i % 2 );++$i;?><div class="d-word">
					<h4><a href="<?php echo url('show', array($ii['id']), 'touch', 'baike'); ?>"><?php echo ($ii["title"]); ?></a></h4>
					<div class="d-tag">
						<?php if(is_array($ii["tagsinfo"])): $i = 0; $__LIST__ = $ii["tagsinfo"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><a href="<?php echo url('agg', array('tag'=>$t['id']), 'touch', 'baike');?>"><?php echo ($t["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
					</div>
				</div><?php endforeach; endif; else: echo "" ;endif; ?>
				<a class="d-more" href="<?php echo url('list', array('id'=>$item['cateid']), 'touch', 'baike'); ?>">查看更多</a><?php endforeach; endif; else: echo "" ;endif; ?>
			<?php else: ?>
				<div class="b_noCon">
					<i></i>
