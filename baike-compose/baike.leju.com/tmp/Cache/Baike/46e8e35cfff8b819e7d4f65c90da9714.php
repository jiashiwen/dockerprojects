...
<div class="z_main_menu">
	...
	<?php if(!empty($binds["bread"])): ?><div class="ty_bread z_bread">
		<?php echo ($binds["bread"]); ?>
	</div><?php endif; ?>
	<div class="ty_left z_side">
		<div class="b_right ty_bright">
			<h2><?php echo ($binds["nav"]["pname"]); ?><i></i></h2>
			<div class="ty_slid">
				<?php if(is_array($binds["nav"]["son"])): $i = 0; $__LIST__ = $binds["nav"]["son"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cc): $mod = ($i % 2 );++$i;?><h3><?php echo ($cc["name"]); ?><i></i></h3>
					<p>
						<?php if(is_array($cc["son"])): $i = 0; $__LIST__ = $cc["son"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ccc): $mod = ($i % 2 );++$i;?><a href="<?php echo url('cate', array('id'=>$ccc['id'], 'page'=>1), 'pc', 'baike');?>"><?php if(($ccc["id"]) == $custom_id): ?><strong style="color:#df1830;"><?php echo ($ccc["name"]); ?></strong><?php else: echo ($ccc["name"]); endif; ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
					</p><?php endforeach; endif; else: echo "" ;endif; ?>
			</div>

			<?php  if ( $binds['nav']['pid'] == 1 ) { ?>
			<div class="ty_slid">
				<h3><a href="javascript:;">特色新房</a><a href="<?php echo ($extra["features"]["moreurl"]); ?>" class="z_more">更多</a></h3>
				<p>
				<?php foreach ( $extra['features']['list'] as $i => $item ) { ?>
				<a href="<?php echo ($item["url"]); ?>"><?php echo ($item["name"]); ?></a>
				<?php } ?>
				</p>
			</div>
			<div class="ty_slid">
				<h3><a href="javascript:;">免费专车看房</a><a href="<?php echo ($extra["cars"]["moreurl"]); ?>" class="z_more">更多</a></h3>
				<ul class="z_house_list">
					<?php foreach ( $extra['cars']['list'] as $i => $car ) { ?>
					<li>
						<h4><a href="<?php echo ($car["url"]); ?>" target="_blank"><?php echo ($car["name"]); ?></a></h4>
						<h5>
							<span class="info">
								<a href="<?php echo ($car["url"]); ?>" target="_blank" class="name"><?php echo ($car["district_name"]); ?></a>|
								<i><?php echo ($car["price_display"]); ?></i>
							</span>
							<a href="<?php echo ($car["url"]); ?>" target="_blank" class="btn">预约</a>
						</h5>
					</li>
					<?php } ?>
				</ul>
			...
			<?php  if ( $binds['nav']['pid'] == 2 ) { ?>
			<?php  if ( $catepath[2]==64 ) { ?>
			<div class="ty_slid">
				<h3><a href="javascript:;">买房搜索</a><a href="<?php echo ($extra["search"]["moreurl"]); ?>" target="_blank" class="z_more">更多</a></h3>
				<p>
				<?php
 foreach ( $extra['search']['list'] as $group_type => $group ) { foreach ( $group as $_i => $item ) { ?>
					<a href="<?php echo ($item["url"]); ?>" data-type="<?php echo ($group_type); ?>" target="_blank"><?php echo ($item["name"]); ?></a>
				<?php
 } } ?>
				</p>
			</div>
			<div class="ty_slid">
				<h3><a href="javascript:;">小区排行</a><a href="<?php echo ($extra["homerank"]["moreurl"]); ?>" target="_blank" class="z_more">更多</a></h3>
				<ul class="z_rank_list">
				<?php foreach ( $extra['homerank']['list'] as $i => $item ) { ?>
					<li>
						<span class="info">
							<a href="<?php echo ($item["url"]); ?>" target="_blank" class="name"><?php echo ($item["communityname"]); ?></a>
							<a href="<?php echo ($item["url"]); ?>" target="_blank" class="address"><?php echo ($item["district"]); ?>-<?php echo ($item["block"]); ?></a>
						</span>
						<a href="<?php echo ($item["url"]); ?>" target="_blank" class="num"><?php echo ($item["salecount"]); ?>套</a>
					</li>
				<?php } ?>
				</ul>
			</div>
			...
			<?php  if ( $catepath[2]==66 ) { ?>
			<div class="ty_slid">
				<h3><a href="javascript:;">租房搜索</a><a href="<?php echo ($extra["search"]["moreurl"]); ?>" target="_blank" class="z_more">更多</a></h3>
				<p>
				<?php
 foreach ( $extra['search']['list'] as $group_type => $group ) { foreach ( $group as $_i => $item ) { ?>
					<a href="<?php echo ($item["link"]); ?>" data-type="<?php echo ($group_type); ?>" target="_blank"><?php echo ($item["name"]); ?></a>
				<?php
 } } ?>
				</p>
			</div>
			<div class="ty_slid">
				<h3><a href="javascript:;">工作聚集</a><a href="<?php echo ($extra["work"]["moreurl"]); ?>" target="_blank" class="z_more">更多</a></h3>
				<ul class="z_rank_list">
				<?php foreach ( $extra['work']['list'] as $i => $item ) { ?>
					<li>
						<span class="info">
							<a href="<?php echo ($item["url"]); ?>" target="_blank" class="name"><?php echo ($item["block"]); ?></a>
							<a href="<?php echo ($item["url"]); ?>" target="_blank" class="address"><?php echo ($item["district"]); ?></a>
						</span>
						<a href="<?php echo ($item["url"]); ?>" target="_blank" class="num"><?php echo ($item["count"]); ?>套</a>
					</li>
				<?php } ?>
				</ul>
			</div>
	...
	<div class="ty_right">
		<h2 class="ty_tit">
			<i></i><span><?php echo ($binds["cate"]); ?></span>
		</h2>
		<div class="ty_list">
		<?php if(!empty($list)): ?><ul>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li>
					<div class="pic">
						<a target="_blank" href="<?php echo url('show', array('id'=>$item['id']), 'pc', 'baike');?>">
							<img src="<?php echo (changeImageSize($item["cover"], 208, 156)); ?>" alt="<?php echo ($item["title"]); ?>">
						</a>
						<a href="<?php echo url('cate', array('id'=>$item['cateid'], 'page'=>1), 'pc', 'baike');?>" class="z_lab" target="_blank"><?php echo ($item["catename"]); ?></a>
					</div>
					<div class="z_r">
					<h3><a target="_blank" href="<?php echo url('show', array('id'=>$item['id']), 'pc', 'baike');?>"><?php echo ($item["title"]); ?></a></h3>
					<p class="pc"><?php echo ($item["content"]); ?></p>
					<p class="pa">
						<?php if(is_array($item["tagsinfo"])): $i = 0; $__LIST__ = $item["tagsinfo"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><a target="_blank" href="<?php echo url('agg', array('tag'=>$t['id'], 'id'=>$cateid, 'page'=>1), 'pc', 'baike');?>"><?php echo ($t["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
					</p>
					</div>
				</li><?php endforeach; endif; else: echo "" ;endif; ?>
			</ul>
		<?php else: ?>
			<div class="ty_noneknow">
				<i class="ty_NKIcon"></i>
				<p>我考虑一下放什么内容好，你先看看其他栏目内容</p>
			</div><?php endif; ?>
		</div>
		<?php if(($binds["total"]) > "0"): ?><div class="ty_pages clearfix">
			...
			<span>共<?php echo ($binds["total"]); ?>页</span>
		</div><?php endif; ?>
	</div>
</div>
<!-- 页尾 -->
