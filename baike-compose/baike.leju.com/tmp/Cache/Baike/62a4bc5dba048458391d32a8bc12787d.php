...
<div class="z_main_menu">
	...
	<?php if(!empty($binds["parent"])): ?><div class="ty_bread">
			<a target="_blank" href="<?php echo url('index', array(), 'pc', 'baike'); ?>" title="房产百科">房产百科</a><i></i>
			<a class="on"><?php echo ($binds["parent"]); ?></a>
		</div><?php endif; ?>
	...
	<div class="ty_right">
		<h2 class="ty_tit">
			<i></i><span>标签 “<strong style="color:red;"><?php echo ($binds["parent"]); ?></strong>” 相关知识</span>
		</h2>
		<div class="ty_list">
		<?php if(!empty($list)): ?><ul>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li>
					...
					<p class="pa">
						<?php if(is_array($item["tagsinfo"])): $i = 0; $__LIST__ = $item["tagsinfo"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><a href="<?php echo url('agg', array('tag'=>$t['id'], 'id'=>$cateid, 'page'=>1), 'pc', 'baike');?>"><?php echo ($t["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
					</p>
					</div>
				</li><?php endforeach; endif; else: echo "" ;endif; ?>
			</ul>
